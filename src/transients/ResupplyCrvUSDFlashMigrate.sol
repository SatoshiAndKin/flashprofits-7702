...
contract ResupplyCrvUSDFlashMigrate is OnlyDelegateCall, IERC3156FlashBorrower {
    ...
    bytes32 internal constant _IN_FLASHLOAN_SLOT =
        keccak256(
            abi.encode(
                uint256(
                    keccak256(
                        "flashprofits.eth.foundry-7702.ResupplyCrvUSDFlashMigrate.in_flashloan"
                    )
                ) - 1
            )
    ...
    function flashLoan(
        ...
        uint256 userCollateralShares = _sourceMarket.userCollateralBalance(
            self
        );
        uint256 collateralValue = collateral.convertToAssets(
    ...
    function onFlashLoan(
        ...
        TransientSlot.BooleanSlot in_on_flashloan = _IN_ON_FLASHLOAN_SLOT
            .asBoolean();
